...
describe("Search", () => {
	...
	context("Responsive view", () => {
		...
		it.skip("should allow user to search by geocode", { scrollBehavior: false }, () => {
			cy.get('[data-testid="search-box-input"]').click();
			cy.wait(2000);
		...
		it.skip("should allow user to view the POIs after a search", { scrollBehavior: false }, () => {
			// eslint-disable-next-line @typescript-eslint/no-unused-vars, no-unused-vars
			Cypress.on("uncaught:exception", (err, runnable) => {
				return false;
			});
		...
		it.skip("should show user an error message when no search text is present", { scrollBehavior: false }, () => {
			cy.get('[data-testid="search-box-input"]').click();
			cy.wait(2000);
			cy.get('[data-testid="search-box-input"]').type("{enter}");
			cy.wait(2000);
