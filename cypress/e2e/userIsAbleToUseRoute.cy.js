...
describe("Route", () => {
	...
	context("Responsive view", () => {
		...
		it.skip("should allow user to swap the departure and destination addresses", { scrollBehavior: false }, () => {
			cy.get('[placeholder="From"]').type("Empire State Building");
			cy.wait(5000);
			cy.contains("Empire State Building").click();
			cy.wait(5000);
		...
		it.skip("should allow user to select any of the route modes from car, walk and truck", () => {
			cy.get(".travel-mode").should("have.class", "selected");
			cy.get(".travel-mode").should("exist").and("have.length", 3);
		});

		it.skip("should allow user to search for route with avoid toll route option", () => {
			cy.get('[placeholder="From"]').click().type("auburn sydney");
			cy.wait(5000);
			cy.contains("Auburn").click();
			cy.wait(5000);
		...
		it.skip("should allow user to search for route with avoid ferry option", () => {
			cy.get('[placeholder="From"]').click().type("port fouad");
			cy.wait(5000);
			cy.contains("Port Fouad").click();
			cy.wait(5000);
